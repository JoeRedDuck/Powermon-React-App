...
print(
    f"Connecting to: postgresql://{DB_USER}:***@{DB_HOST}:{DB_PORT}/{DB_NAME}")
...
try:
    ...
    session.execute(
        text(f"DELETE FROM poll WHERE machine_name IN ('{old_name}', '{new_name}')"))
    session.execute(
        text(f"DELETE FROM monitor WHERE monitor_mac_address = '{test_mac}'"))
    session.execute(
        text(f"DELETE FROM machine WHERE machine_name IN ('{old_name}', '{new_name}')"))
    session.commit()
    ...
    print(
        f"✓ Verified: machine={machine.name}, monitor FK={monitor.machine_name}, polls={len(polls)}")
    ...
    try:
        ...
        old_machine = session.query(
            models.Machine).filter_by(name=old_name).first()
        new_machine = session.query(
            models.Machine).filter_by(name=new_name).first()
        ...
        if monitor.machine_name != new_name:
            print(
                f"✗ FAILED: Monitor FK still points to '{monitor.machine_name}'")
            sys.exit(1)

        print(f"✓ Monitor FK updated to '{new_name}'")

        # Check polls were updated
        old_polls = session.query(models.Poll).filter_by(
            machine_name=old_name).all()
        new_polls = session.query(models.Poll).filter_by(
            machine_name=new_name).all()

        if len(old_polls) > 0:
            print(
                f"✗ FAILED: {len(old_polls)} polls still reference '{old_name}'")
            sys.exit(1)

        if len(new_polls) != 2:
            print(
                f"✗ FAILED: Expected 2 polls for '{new_name}', found {len(new_polls)}")
            sys.exit(1)
        ...
        if len(constraints) != 2:
            print(
                f"✗ FAILED: Expected 2 FK constraints, found {len(constraints)}")
            sys.exit(1)
